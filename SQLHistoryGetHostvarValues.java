...
private static void generateTIAULFile( List<String> sqlForDSNTIAUL,int sanitizedFileNum, int subSanitizedFileNum, String pathToSQLHistory){
   ...
   String dsntiaulStep =
	...
	BufferedWriter writerTEP2 = new BufferedWriter(new FileWriter(pathToSQLHistory+ System.getProperty("file.separator")+outputTEP2filename));
	...
	String command = "fold -w72 -s "+pathToSQLHistory+ System.getProperty("file.separator")+outputTEP2filename+ " > "+pathToSQLHistory+ System.getProperty("file.separator")+tiaulFilename;
	...
	Files.deleteIfExists(Paths.get(pathToSQLHistory+ System.getProperty("file.separator")+outputTEP2filename));
  } catch (Exception e) {
	 e.printStackTrace();
  }
 }
   ...
   public static void main(String[] args) throws IOException {
	...
	FilenameFilter filterSanitized = new FilenameFilter() {
      ...
      try {
         ...
         if (args.length==2){
		 ...
		 }else if (args.length>=3 && args[2].equals("sanitizeit")){ //use this option if the default execution shows XML parser invalid character errors
			 ...
			 for (int i = 0; i < listOfSQLHistoryFiles.length; i++) {
				 ...
				 BufferedWriter writerSanitizedSQLHistory = new BufferedWriter(new FileWriter(pathToSQLHistory+ System.getProperty("file.separator")+outputSanitizedSQLHistoryfilename));
				 ...
				 while ((line = in.readLine()) != null) {
					...
					}else if (sanitizedline.contains("parameterMarkerValues") == true)	// parameter marker value could be %A & %B, the & character messes up the xml parser, ok to ignore here
					{
						...
						case XMLStreamConstants.START_ELEMENT:
							if (reader.getLocalName().equals( "SQLStatementDetails" ) ) {
								if(reader.getAttributeCount() > 0) {
									...
									mSummaryString = taskID+"," +netezzaExecTime+"," +totalElapsedTime+","+totalWaitTime+","+numResultRows+","+numResultBytes+","+fetchTime+",'"+entryTimestamp+"','"+originalUserID+"',";
								...
								int clientSQLQueryEventType = reader.next();
								if (!finishState.equals("CANCELED") && (clientSQLQueryEventType == XMLStreamConstants.CHARACTERS || clientSQLQueryEventType == XMLStreamConstants.CDATA)){
									String db2zSQLStmt = removeComments(reader.getText());
									String newDb2zSQLStmt = replacePMWithValues(clientSQLQuery,parameterMarkerValues,db2zSQLStmt);
									db2zSQLStmt = newDb2zSQLStmt.replaceAll("\t", "        ");

									Pattern patternWithUR = Pattern.compile("WITH\\s*UR", Pattern.CASE_INSENSITIVE);
									Pattern patternNewlineWithUR = Pattern.compile("\nWITH\\s*UR", Pattern.CASE_INSENSITIVE);
